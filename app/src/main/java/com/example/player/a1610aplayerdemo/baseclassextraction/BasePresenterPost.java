...
public abstract class BasePresenterPost<T> implements IBasePresenter<T>{

    protected IBaseView<T> iBaseView;

    @Override
    public void loadDataByGet() {
        RetrofitCreatorUse.getApiService().getLogin(getHeaders(),getAPIPath1(),getAPIPath2(),getParmams())
